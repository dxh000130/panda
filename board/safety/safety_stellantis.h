...
AddrCheckStruct stellantis_addr_checks[] = {
  ...
  {.msg = {{MSG_DASM_ACC, 0, 8, .check_checksum = true, .max_counter = 15U, .expected_timestep = 20000U}, { 0 }, { 0 }}},
};
...
static int stellantis_rx_hook(CAN_FIFOMailBox_TypeDef *to_push) {
  ...
  if (valid && (GET_BUS(to_push) == 0)) {
    ...
    // Measured eps torque
    if (addr == MSG_EPS_2) {
      int torque_meas_new = (((GET_BYTE(to_push, 0) & 0x15U) << 8) | GET_BYTE(to_push, 1)) - 3072U;
    ...
    if (addr == MSG_WHEEL_SPEEDS) {
      int wheel_speed_fl = ((GET_BYTE(to_push, 4) & 0x7U) << 8) | GET_BYTE(to_push, 5);
      int wheel_speed_fr = ((GET_BYTE(to_push, 6) & 0x7U) << 8) | GET_BYTE(to_push, 7);
